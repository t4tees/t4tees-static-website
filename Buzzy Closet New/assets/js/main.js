...
class Tinyx {
	...
	createHTML() {
		...
		this.html = document.querySelector(
			`.js-tinyx[data-tinyx-id="${this.options.id}"]`
		);
	...
	addIndexToImgs() {
		this.imgLinks.forEach((el, i) => {
			el.querySelector("img").setAttribute("data-tinyx-img-index", i);
		});
	}
	...
	prev() {
		...
		this.open(
			...
			this.element.querySelector(
				`[data-tinyx-img-index="${this.imgIndex}"]`
			)
		);
	}
